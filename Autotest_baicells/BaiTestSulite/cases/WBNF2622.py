...
class DoTest():
    ...
    def _dotest(self,index):
        ...
        ipsec1down = job._dosshcmd(self.args["chost"],27149,"route add -host %s gw %s" % (self.ipsecips[index][0],cell_ipsec_errorgw),
                                   "root123",True)
        if not ipsec1down:
        ...
        ipsec1start = job._dosshcmd(self.args["chost"],27149,"route del -host %s " % self.ipsecips[index][0]),
                                   "root123",True)
        if not ipsec1start:
