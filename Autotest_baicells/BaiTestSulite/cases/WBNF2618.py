...
class DoTest():
    ...
    def _dotest(self):
        ...
        if uenum_ping != "1":
            return False
        self.logger.info("set ipsec tunnel1 down")
        cell_ipsec_errorgw = self.args["chost"][:-1] + "100"
        ipsec1down = job._dosshcmd(self.args["chost"],27149,"route add -host %s gw %s" % (self.ipsecips[0][0],cell_ipsec_errorgw),
                                   "root123",True)
        if not ipsec1down:
            self.logger.error("ipsec1 add error route fail")
            return False
        ...
        if uenum_ping != "1":
            return False
        self.logger.info("start ipsec1")
        ipsec1start = job._dosshcmd(self.args["chost"],27149,"route del -host %s " % self.ipsecips[0][0],"root123",True)
        if not ipsec1start:
            self.logger.error("del ipsec1 error route fail")
            return False
        time.sleep(10)
        self.logger.info("stop ipsec2")
        ipsec2down = job._dosshcmd(self.args["chost"],27149,"route add -host %s gw %s" % (self.ipsecips[1][0],cell_ipsec_errorgw),"root123",True)
        if not ipsec2down:
            self.logger.error("ipsec2 add error route fail")
            return False
        self.logger.info("check mme status")
        time.sleep(30)
        test = LmtCheck("http://%s" % self.args["chost"])
        ...
        if uenum_ping != "1":
            return False
        self.logger.info("start ipsec2")
        ipsec2start = job._dosshcmd(self.args["chost"],27149,"route del -host %s " % self.ipsecips[1][0],"root123",True)
        if not ipsec2start:
            self.logger.error("ipsec2 start fail")
            return False
    ...
    def _runtest(self):
        ...
        for i in range(times):
            ...
            if status:
                ...
                failtime +=1
        if failtime > 0:
            self.logger.error("WB.NF.26.18 is test Fail")
            return "Fail"
        else:
            self.logger.info("WB.NF.26.18 is test Pass")
            return "Pass"
