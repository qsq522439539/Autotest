...
class DoTest():
    ...
    def _dotest(self):
        ...
        if uenum_ping != "1":
            return False
        self.logger.info("start mme1")
        self.logger.info("delet error gw")
        job.delete_mme_gw(self.args["chost"],self.mmeips,0)
        if not mme1start:
            self.logger.error("mm1 start fail")
            return False
        return True
    ...
    def _runtest(self):
        ...
        for i in range(times):
            ...
            if status:
                ...
                failtime +=1
        if failtime > 0:
            self.logger.error("WB.NF.26.24 is test Fail")
            return "Fail"
        else:
            self.logger.info("WB.NF.26.24 is test Pass")
            return "Pass"
