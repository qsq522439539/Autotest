...
class DoTest():
    ...
    def _dotest(self):
        ...
        test.login_lmt()
        status = test.capacity_parameters_check("1")
        if status:
        ...
        test = LmtCheck("http://%s" %self.args["chost"])
        test.login_lmt()
        status = test.capacity_parameters_check("0")
        if status:
    ...
    def _runtest(self):
        ...
        for i in range(times):
            ...
            if status:
                ...
                failtime +=1
        if failtime > 0:
            self.logger.error("BS1105_DD_031.2 is test Fail")
            return "Fail"
        else:
            self.logger.info("BS1105_DD_031.2 is test Pass")
            return "Pass"
